...
| `dev/{id}/wipe` | backend → ESP | 1 | **no** | `"1"` | สั่ง ESP ลืม id แล้ว re-pair (ดู §Remove device flow) |
...
services/devices.ts:removeDevice
   ...
   └ mqtt.publishWipe(id)                     pub dev/{id}/wipe   '1' qos1 (non-retained)
```

**Why clear 3 topics + publish wipe:**
...
- `dev/{id}/wipe` — สั่ง ESP ที่ยัง paired ลืม id แล้ว boot เข้า pairing mode

### Auto re-pair (ESP-side wipe)

ESP ที่ paired subscribe `dev/{id}/wipe` พร้อม `dev/{id}/cmd`. รับ message ใดๆ บน wipe topic → `LittleFS.remove("/id.txt")` → `ESP.restart()` → boot ในโหมด pairing → publish `pair/hello` ใหม่

**2 trigger ฝั่ง backend:**
1. **เคส ESP online ตอนลบ** — `removeDevice` publish wipe ทันที (non-retained QoS1)
2. **เคส ESP offline ตอนลบ** — ESP กลับมา online → publish hb → backend `handleHeartbeat` เห็น `!doc` → publish wipe กลับ (debounce 60s ต่อ id กัน log spam)

**Non-retained ตั้งใจ:** retained จะทำให้ ESP ที่ re-pair กลับมา (ID เดิมเพราะ MAC เดิม) subscribe `dev/{id}/wipe` แล้วได้ retained wipe → wipe ซ้ำ → infinite loop. Non-retained หลีกเลี่ยงปัญหานี้
